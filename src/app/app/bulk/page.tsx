...
export default async function TemplatesPage({
  ...
  async function create(formData: FormData) {
    ...
    const ws = wsList?.[0];
    if (!ws) redirect("/app?status=NO_WORKSPACE");
  ...
  const st = statusMessage(searchParams.status ?? "");
  return (
    <div className="space-y-6 max-w-2xl">
      ...
      <form action={create} className="border rounded p-4 space-y-3">
        <input name="name" placeholder="Nome (ex: Cliente X - IG Ads)" className="border rounded p-2 w-full" />
        <input name="base_url" placeholder="Base URL (https://...)" className="border rounded p-2 w-full" />
        ...
        <input name="utm_campaign_prefix" placeholder="Prefixo campanha (opcional)" className="border rounded p-2 w-full" />
      ...
      <div className="space-y-2">
        {(templates ?? []).map((t) => (
          <div key={t.id} className="border rounded p-3">
            ...
            {t.utm_campaign_prefix && <div className="text-sm text-gray-600">prefix={t.utm_campaign_prefix}</div>}
          </div>
        ))}
